...
# Group and clean sector funding data
top_sectors = df.groupby('vertical')['amount'].sum().sort_values(ascending=False)

# Remove invalid entries
top_sectors = top_sectors.dropna()
top_sectors = top_sectors[top_sectors.index != 'Unknown']
top_sectors = top_sectors[top_sectors > 0].head(5)

# Plot or fallback
if top_sectors.empty:
    st.warning("No valid sector funding data available to plot.")
else:
